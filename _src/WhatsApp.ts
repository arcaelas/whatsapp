...
import Store, { FileStore } from './store';
import { Engine } from './types';

export interface IWhatsApp {
    /** Engine de persistencia. Por defecto usa FileStore en `.baileys/{phone}/` o `.baileys/default/`. */
    engine?: Engine;
    /** Numero para autenticacion por codigo (opcional, sin +) */
    phone?: string;
}
...
/**
 * @description Clase principal para gestionar la conexion con WhatsApp.
 * @example
 * const wa = new WhatsApp({ phone: '5491155555555' })
 * wa.on('message:created', (msg) => console.log('Nuevo:', msg.id))
 * await wa.pair((code) => console.log('Ingresa:', code))
 */
export class WhatsApp extends EventEmitter<{ [K in keyof WhatsAppEventMap]: [WhatsAppEventMap[K]] }> {
    ...
    constructor(options: IWhatsApp = {}) {
        super();
        const engine = options.engine ?? new FileStore(options.phone ? `.baileys/${options.phone}` : '.baileys/default');
        this._store = new Store(engine);
        ...
        this._setup_storage_handlers();
    }

    /**
     * @description Inicia autenticacion. Modo QR (sin phone) o Code (con phone).
     * @param callback Recibe Buffer PNG (QR) o string de 8 chars (Code)
     */
    async pair(callback?: (data: Buffer | string) => void | Promise<void>): Promise<void> {
        const promise = promify();
        const { version } = await fetchLatestBaileysVersion();
        const [creds] = await this._store.document.get<AuthenticationCreds>('session/creds');
            ...
            auth: {
                ...
                keys: {
                    get: async (type, ids) => {
                        ...
                        for (const id of ids) {
                            let [value] = await this._store.document.get(`session/signal/${type}/${id}`);
                    ...
                    set: async (data) => {
                        for (const type in data) {
                            for (const id in data[type]) {
                                await this._store.document.set(`session/signal/${type}/${id}`, data[type][id] ?? null);
                            }
                        }
                    },
                },
        ...
        sock.ev.on('connection.update', async ({ connection, lastDisconnect, qr }: Partial<ConnectionState>) => {
            if (qr && callback) {
                if (this._phone && !code_sent) {
                    code_sent = true;
                    await callback(await sock.requestPairingCode(this._phone));
                } else if (!this._phone) {
        ...
        sock.ev.on('creds.update', () => this._store.document.set('session/creds', sock.authState.creds));
        ...
        sock.ev.on('contacts.upsert', (list) => list.forEach((c) => c.id && this._emit_contact(c)));
        sock.ev.on('contacts.update', (list) => list.forEach((c) => c.id && this._emit_contact(c)));
        sock.ev.on('chats.upsert', (list) => list.forEach((ch) => this._emit_chat(ch)));

        sock.ev.on('chats.update', async (list) => {
            for (const u of list) {
                if (!u.id) continue;
                const existing = await this._store.chat.get(u.id);
                if (existing) this._emit_chat({ ...existing, name: (u as { name?: string }).name ?? existing.name });
            }
        });

        sock.ev.on('chats.delete', (ids) => {
            ids.forEach((id) => this.emit('chat:deleted', id));
        });

        sock.ev.on('messages.upsert', async ({ messages }) => {
            for (const m of messages) {
                getContentType(m.message ?? {}) === 'pollUpdateMessage'
                    ? await this._process_poll_vote(m)
                    : this._emit_message(m);
            }
        });

        sock.ev.on('messages.update', async (list) => {
            for (const { key, update } of list) {
                if (!key.remoteJid || !key.id) continue;
                const existing = await this._store.message.get(key.remoteJid, key.id);
                if (!existing) continue;

                // Determinar si es edicion o cambio de estado
                const is_edit = !!(update as { edit?: unknown }).edit;
                const merged: IMessage = {
                    ...existing,
                    status: update.status != null ? (STATUS_MAP[update.status] ?? existing.status) : existing.status,
                    edited: is_edit || existing.edited,
                };

                const instance = new this.Message(merged);
                instance.content = async () => (await this._store.content.get(merged.cid, merged.id)) ?? Buffer.alloc(0);
        ...
        sock.ev.on('messages.delete', async (del) => {
            if ('all' in del && del.all) {
                const messages = await this._store.message.find(del.jid, 0, 1000);
                for (const m of messages) {
                    this.emit('message:deleted', { cid: del.jid, mid: m.id });
                }
            } else if ('keys' in del) {
                for (const k of del.keys) {
                    if (k.remoteJid && k.id) {
                        this.emit('message:deleted', { cid: k.remoteJid, mid: k.id });
                    }
                }
            }
        });
        ...
        sock.ev.on('messages.reaction', async (reactions) => {
            for (const { key } of reactions) {
                ...
                instance.content = async () => (await this._store.content.get(existing.cid, existing.id)) ?? Buffer.alloc(0);
    ...
    /**
     * @description Espera sincronizacion completa.
     * @param callback Progreso 0-100
     */
    ...
    // ─────────────────────────────────────────────────────────────────────────────
    // PRIVATE: Storage handlers (primer listener de cada evento)
    // ─────────────────────────────────────────────────────────────────────────────

    private _setup_storage_handlers(): void {
        this.on('contact:upsert', async (i) => {
            const e = await this._store.contact.get(i.id);
            await this._store.contact.set({
                id: i.id,
                name: i.name,
                phone: i.phone,
                photo: i.photo ?? e?.photo ?? null,
                custom_name: e?.custom_name ?? i.custom_name,
            });
        });

        this.on('chat:upsert', async (i) => {
            const e = await this._store.chat.get(i.id);
            await this._store.chat.set({
                id: i.id,
                name: i.name,
                phone: i.phone,
                photo: i.photo ?? e?.photo ?? null,
        ...
        this.on('chat:deleted', async (cid) => {
            await this._store.chat.delete(cid);
        });

        this.on('message:created', async (i) => {
            await this._store.message.set({
                id: i.id,
                cid: i.cid,
                uid: i.uid,
                mid: i.mid,
            ...
            if (content.length) await this._store.content.set(i.cid, i.id, content);
        });
        ...
        this.on('message:deleted', async ({ cid, mid }) => {
            await this._store.message.delete(cid, mid);
        });
    }

    // ─────────────────────────────────────────────────────────────────────────────
    // PRIVATE: Transformadores (Baileys → instancia con content virtualizado)
    // ─────────────────────────────────────────────────────────────────────────────
    ...
    private _emit_contact(c: Partial<BaileysContact>): void {
        ...
        this.emit(
            ...
            new this.Contact({
                ...
                name,
                phone: id.split('@')[0],
                photo: c.imgUrl === 'changed' ? null : (c.imgUrl ?? null),
                custom_name: name,
            })
        );
    }
    ...
    private _emit_chat(ch: Partial<BaileysChat> & { id: string }): void {
        ...
        this.emit(
            ...
            new this.Chat({
                ...
                photo: null,
                phone: is_group ? null : ch.id.split('@')[0],
                type: is_group ? 'group' : 'contact',
            })
        );
    }
    ...
    private _emit_message(msg: proto.IWebMessageInfo): void {
        ...
        instance.content = this._create_content_getter(msg_type, raw, msg);
        this.emit('message:created', instance);
    }

    private _create_content_getter(
        type: IMessage['type'],
        ...
        if (type === 'text') {
            ...
            return async () => Buffer.from(text);
        }
        if (type === 'location') {
            return async () => Buffer.from(JSON.stringify({ lat: raw?.degreesLatitude, lng: raw?.degreesLongitude }));
        }
        if (type === 'poll') {
            ...
            return async () =>
                Buffer.from(
                    JSON.stringify({
                        content: p?.name ?? '',
                        items: p?.options?.map((o) => ({ content: o.optionName, voters: [] })) ?? [],
                        sign: p?.encKey ? Buffer.from(p.encKey).toString('base64') : null,
                    })
                );
        }
        if (['image', 'video', 'audio'].includes(type)) {
            return async () => {
                if (!this._socket) return Buffer.alloc(0);
                try {
    ...
    private async _process_poll_vote(msg: proto.IWebMessageInfo): Promise<void> {
        ...
        const buffer = await this._store.content.get(key.remoteJid, key.id);
        if (!buffer) return;
        ...
        try {
            ...
            const decrypted = decryptPollVote(update.vote, {
                pollCreatorJid: jidNormalizedUser(key.participant ?? key.remoteJid),
                pollMsgId: key.id,
                pollEncKey: Buffer.from(poll.sign, 'base64'),
                voterJid: voter,
            });
            ...
            for (const item of poll.items) {
                ...
                if (selected.has(createHash('sha256').update(item.content).digest('hex'))) {
                    item.voters.push(voter);
                }
            }
            await this._store.content.set(key.remoteJid, key.id, Buffer.from(JSON.stringify(poll)));
        } catch {
            // Error descifrando voto
        }
    }
}
